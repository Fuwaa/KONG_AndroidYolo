...
//    private MainActivity mainActivity; // 新增的成员变量
//
...
//                             MainActivity mainActivity) { // 修改构造方法
...
//        this.mainActivity = mainActivity; // 初始化成员变量
//    }
//
//    @Override
...
//
//                        detectedItems.append(label).append(" ");
//                    }
//
//                    if (detectedItems.length() > 0) {
//                        mainActivity.speakOut(detectedItems.toString().trim());
//                    }
//
...
public class FullScreenAnalyse implements ImageAnalysis.Analyzer {
    ...
    private Yolov5TFLiteDetector yolov5TFLiteDetector;
    private MainActivity mainActivity; // 添加 MainActivity 实例

    public FullScreenAnalyse(MainActivity mainActivity, PreviewView previewView, ImageView boxLabelCanvas, int rotation,
                             TextView inferenceTimeTextView, TextView frameSizeTextView, Yolov5TFLiteDetector yolov5TFLiteDetector) {
        this.previewView = previewView;
        this.boxLabelCanvas = boxLabelCanvas;
        this.rotation = rotation;
        this.inferenceTimeTextView = inferenceTimeTextView;
    ...
    public void analyze(@NonNull ImageProxy image) {
        ...
        Observable.create((ObservableEmitter<Result> emitter) -> {
                    ...
                    Matrix previewToModelTransform = imageProcess.getTransformationMatrix(
                            cropImageBitmap.getWidth(), cropImageBitmap.getHeight(),
                            yolov5TFLiteDetector.getInputSize().getWidth(), yolov5TFLiteDetector.getInputSize().getHeight(),
                            0, false);
                    Bitmap modelInputBitmap = Bitmap.createBitmap(cropImageBitmap, 0, 0,
                            cropImageBitmap.getWidth(), cropImageBitmap.getHeight(),
                            previewToModelTransform, false);
